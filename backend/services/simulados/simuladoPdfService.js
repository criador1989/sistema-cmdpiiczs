...
let geracaoPdfAtiva = false;

function erroPdfOcupado() {
  const error = new Error('Já existe um relatório PDF sendo gerado. Aguarde a conclusão e tente novamente.');
  error.codigo = 'PDF_OCUPADO';
  return error;
}

async function comGeracaoPdfExclusiva(fn) {
  if (geracaoPdfAtiva) throw erroPdfOcupado();
  geracaoPdfAtiva = true;
  try {
    return await fn();
  } finally {
    geracaoPdfAtiva = false;
  }
}
...
async function gerarPdfArquivoComModo({ simulado, dashboard, resultados = [], comparacao = null, turma = '', modoRelatorio = 'geral' }) {
  return comGeracaoPdfExclusiva(async () => {
    ...
    try {
      ...
      try {
        ...
        await handle.read(assinatura, 0, 5, 0);
        if (stat.size < 1500 || assinatura.toString('ascii') !== '%PDF-') {
...
async function gerarPdfComModo(args) {
  const arquivo = await gerarPdfArquivoComModo(args);
  try {
    return await fs.readFile(arquivo.caminho);
  } finally {
    await arquivo.limpar();
  }
}

async function gerarRelatorioDiagnosticoPdfArquivo(args) {
  return gerarPdfArquivoComModo({ ...args, modoRelatorio: 'geral' });
}

async function gerarRelatorioVisualPdfArquivo(args) {
  return gerarPdfArquivoComModo({ ...args, modoRelatorio: 'visual', resultados: [] });
}

async function gerarRelatorioHabilidadesEnemPdfArquivo(args) {
  ...
  return gerarPdfArquivoComModo({ ...args, modoRelatorio: 'habilidades_enem', resultados: [] });
}
