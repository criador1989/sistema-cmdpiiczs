...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!usuario) return res.status(401).json({ mensagem: 'Usuário não encontrado ou inativo.' });
...
router.post('/cadastrar', autenticar, async (req, res) => {
  ...
  const { nome, email, senha, instituicao, tipo = 'monitor' } = req.body;
  ...
  try {
    ...
    if (existente) return res.status(409).json({ mensagem: 'E-mail já está em uso nesta instituição.' });

    const novoUsuario = new Usuario({
      nome,
      email: String(email).trim().toLowerCase(),
      senha,        // hash feito no pre('save') do model
      tipo,
      instituicao,
    });
