...
def nivel_label(value: Any) -> str:
    ...
    return {
        ...
        'prioritario': 'Prioridade pedagógica',
...
def nivel_aluno_label(value: Any) -> str:
    """Rótulo específico para situação/classificação de estudantes.

    'Evidência insuficiente' continua válido para itens e habilidades, mas no
    nível do estudante a expressão operacional é 'Base incompleta', evitando
    misturar ausência confirmada, qualidade do item e suficiência da base
    individual.
    """
    chave = txt(value).lower()
    if chave == 'evidencia_insuficiente':
        return 'Base incompleta'
    if chave == 'critico':
        return 'Intervenção imediata'
    return nivel_label(value)


def faixas_operacionais_alunos(cfg: dict[str, Any]) -> list[tuple[str, str, str, str]]:
    prioritario = num(cfg.get('percentualAtencao', 50), 50)
    consolidado = num(cfg.get('percentualConsolidado', 70), 70)
    critico = prioritario * 0.5
    prioridade_alta = prioritario * 0.8
    return [
        ('critico', 'Intervenção imediata', f'abaixo de {pct(critico)}', 'Resultado muito baixo neste recorte; requer retomada prioritária e conferência com outras evidências.'),
        ('prioridade_alta', 'Prioridade alta', f'{pct(critico)} a < {pct(prioridade_alta)}', 'Predominam dificuldades no conjunto avaliado; recomenda intervenção planejada.'),
        ('em_atencao', 'Em atenção', f'{pct(prioridade_alta)} a < {pct(prioritario)}', 'Resultado próximo do limiar de prioridade, ainda com fragilidades relevantes.'),
        ('em_desenvolvimento', 'Em desenvolvimento', f'{pct(prioritario)} a < {pct(consolidado)}', 'Há aprendizagem demonstrada, mas ainda sem consistência suficiente para a faixa consolidada.'),
        ('consolidado', 'Consolidado', f'a partir de {pct(consolidado)}', 'Desempenho consistente neste simulado; não significa domínio definitivo nem proficiência TRI.'),
    ]


def legenda_classificacao_alunos(distribuicao: list[dict[str, Any]], cfg: dict[str, Any]) -> Table:
    contagens = {txt(x.get('nivel')).lower(): inteiro(x.get('quantidade')) for x in (distribuicao or [])}
    celulas = []
    for chave, rotulo, regra, significado in faixas_operacionais_alunos(cfg):
        celulas.append([
            Paragraph(f'<b>{html(rotulo)}</b>', SMALL_CENTER),
            Paragraph(html(regra), SMALL_CENTER),
            Paragraph(f'<b>{contagens.get(chave, 0)}</b> estudante(s)', SMALL_CENTER),
            Paragraph(html(significado), SMALL_CENTER),
        ])
    largura = (PAGE_W - 28 * mm) / 5
    t = Table([celulas], colWidths=[largura] * 5, hAlign='LEFT')
    comandos = [
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
    ...
    for idx, (chave, _, _, _) in enumerate(faixas_operacionais_alunos(cfg)):
        comandos += [
            ('BACKGROUND', (idx, 0), (idx, 0), nivel_bg(chave)),
            ('BOX', (idx, 0), (idx, 0), 0.6, LINE),
        ]
    t.setStyle(TableStyle(comandos))
    return t


def tabela_faixas_estudantes(cfg: dict[str, Any]) -> Table:
    data = [['Faixa', 'Intervalo', 'Como interpretar']]
    for _chave, rotulo, regra, significado in faixas_operacionais_alunos(cfg):
        data.append([rotulo, regra, significado])
    return table(data, [42*mm, 34*mm, 106*mm], font_small=True)


def tabela_faixas_questoes(cfg: dict[str, Any]) -> Table:
    minimo = inteiro(cfg.get('minimoRespondentesQuestao', 5))
    return table([
        ['Faixa da questão', 'Desempenho confirmado', 'Como interpretar'],
        ['Muito baixo', 'abaixo de 20%', 'Poucos acertos no item; investigar conteúdo, comando, distratores e ensino associado.'],
        ['Baixo', '20% a < 40%', 'Dificuldade relevante no item.'],
        ['Intermediário', '40% a < 60%', 'Item divide o grupo; requer leitura junto ao conteúdo e aos distratores.'],
        ['Alto', '60% a < 80%', 'Maioria acertou o item.'],
        ['Muito alto', '80% ou mais', 'Item com elevado percentual de acertos.'],
        ['Evidência insuficiente', f'menos de {minimo} respondentes', 'Não classificar o item coletivamente com uma base tão pequena.'],
    ], [42*mm, 42*mm, 98*mm], font_small=True)


def tabela_parametros_diagnostico(cfg: dict[str, Any]) -> Table:
    atencao = num(cfg.get('percentualAtencao', 50), 50)
    consolidado = num(cfg.get('percentualConsolidado', 70), 70)
    min_q = inteiro(cfg.get('minimoQuestoesIndicador', 2))
    min_resp = inteiro(cfg.get('minimoRespondentesQuestao', 5))
    min_grupo = inteiro(cfg.get('minimoAlunosGrupo', 2))
    min_cob = num(cfg.get('minimoCoberturaIndividual', 80), 80)
    return table([
        ['Parâmetro interno', 'Valor', 'Por que existe'],
        ['Prioridade pedagógica', f'abaixo de {pct(atencao)}', 'Limiar operacional para sinalizar desempenho inferior à metade dos pontos quando configurado em 50%. Não é corte oficial do ENEM.'],
        ['Consolidado', f'a partir de {pct(consolidado)}', 'Marca desempenho mais consistente no recorte. Não significa domínio definitivo da habilidade nem nota TRI.'],
        ['Mínimo de questões por indicador', str(min_q), 'Evita concluir domínio ou dificuldade de uma habilidade a partir de um único item; um item isolado permanece indicativo.'],
        ['Mínimo de respondentes por questão', str(min_resp), 'Evita leitura coletiva de um item com base extremamente pequena; é uma trava operacional, não garantia estatística de representatividade.'],
        ['Mínimo de alunos no grupo', str(min_grupo), 'Separa necessidade individual de necessidade compartilhada para organizar intervenção; não é parâmetro estatístico.'],
        ['Cobertura mínima individual', pct(min_cob), 'Evita classificação global quando parte relevante da prova está sem dados. Ausência confirmada é tratada separadamente.'],
        ['Subfaixas abaixo da prioridade', f'< {pct(atencao*0.5)} / {pct(atencao*0.5)} a < {pct(atencao*0.8)} / {pct(atencao*0.8)} a < {pct(atencao)}', 'Dividem a zona abaixo do limiar prioritário em intervenção imediata, prioridade alta e atenção para ordenar a ação pedagógica. São faixas internas de triagem.'],
    ], [52*mm, 45*mm, 85*mm], font_small=True)
...
def chart_histograma(items: list[dict[str, Any]], width=175*mm, height=60*mm) -> Drawing:
    data = list(items or [])
    ...
    if not data:
        ...
        return d
    left=12*mm; bottom=14*mm; top=6*mm; right=4*mm
    ...
    for idx,item in enumerate(data):
        ...
        d.add(String(x+bw/2, 5.2*mm, str(inteiro(item.get('inicio'))), fontName='Helvetica', fontSize=5.5, fillColor=MUTED, textAnchor='middle'))
        if count:
    ...
    d.add(String(width-right,5.2*mm,'100',fontName='Helvetica',fontSize=5.5,fillColor=MUTED,textAnchor='end'))
    d.add(String((left + width - right)/2,1.0*mm,'Desempenho confirmado (%)',fontName='Helvetica-Bold',fontSize=6,fillColor=MUTED,textAnchor='middle'))
    return d
...
def build_story_visual(payload: dict[str, Any]) -> list:
    ...
    story.append(metric_cards([
        ('Estudantes no recorte', str(inteiro(resumo.get('participantes'))), f"{inteiro(resumo.get('turmas'))} turma(s)"),
        ('Desempenho confirmado', pct(resumo.get('percentualPontuacao')), 'percentual de acertos; brancos valem zero'),
        ('Cobertura das respostas', pct(resumo.get('coberturaPercentual')), 'confirmadas entre as respostas aplicáveis'),
        ('Classificação individual', str(inteiro(resumo.get('alunosBaseAdequada'))), 'participação completa + dados suficientes'),
        ('Participação parcial', str(inteiro(resumo.get('alunosParticipacaoParcial'))), 'faltou a pelo menos um dia; dados realizados seguem válidos'),
        ('Base incompleta', str(inteiro(resumo.get('alunosBaseIncompleta'))), 'participou, mas faltam dados para conclusão global'),
    ], columns=3))
    story.append(Spacer(1, 2*mm))
    story.append(callout(
        'Como ler estes números',
        'Estudantes no recorte é o universo total do simulado/recorte. Cobertura mede respostas, não presença. Participação parcial é ausência confirmada em pelo menos um dia; Base incompleta é insuficiência real de dados para classificar o estudante globalmente.',
        'info',
    ))

    story += section('1. Visão geral por área', 'Compara o percentual de acertos confirmados entre as áreas avaliadas.')
    story.append(callout(
        'Como ler este gráfico',
        f"Cada barra mostra o desempenho confirmado da área: acertos entre os pontos possíveis nas respostas confirmadas; branco vale zero. As referências de {pct(cfg.get('percentualAtencao', 50))} e {pct(cfg.get('percentualConsolidado', 70))} são critérios internos de gestão, não cortes oficiais do ENEM nem nota TRI. Barras menores indicam áreas que merecem maior investigação pedagógica neste recorte.",
        'info',
    ))
    story.append(Spacer(1, 2*mm))
    ...
    total_estudantes = inteiro(resumo.get('participantes'))
    classificados = inteiro(resumo.get('alunosBaseAdequada'))
    parciais = inteiro(resumo.get('alunosParticipacaoParcial'))
    incompletos = inteiro(resumo.get('alunosBaseIncompleta'))
    story += section('2. Distribuição do desempenho individual', 'As faixas de desempenho são aplicadas somente a estudantes com participação completa e dados suficientes para classificação individual.')
    story.append(callout(
        'Universo do gráfico',
        f'{classificados} de {total_estudantes} estudante(s) entram no histograma. Fora do gráfico: {parciais} com participação parcial por ausência confirmada e {incompletos} com base incompleta. Ausência não é transformada em baixo desempenho.',
        'info',
    ))
    if dist:
        story.append(Spacer(1, 2*mm))
        story.append(legenda_classificacao_alunos(dist, cfg))
        story.append(Spacer(1, 2*mm))
    story.append(chart_histograma(visual.get('histogramaDesempenho') or []))
    story.append(p(f"As faixas são critérios internos. O limiar de prioridade é {pct(cfg.get('percentualAtencao', 50))}; abaixo dele o Axoriin subdivide a triagem em três níveis para ordenar a intervenção. A faixa consolidada começa em {pct(cfg.get('percentualConsolidado', 70))}. Nenhuma dessas faixas corresponde a corte oficial do ENEM.", SMALL))
    story.append(Spacer(1, 3*mm))

    story += section('3. Turmas lado a lado', 'Compara desempenho e disponibilidade de dados de cada turma.')
    story.append(callout(
        'Como ler este gráfico',
        f"Barra sólida = desempenho confirmado da turma. Barra azul = cobertura das respostas, isto é, quanto dos dados aplicáveis está confirmado; cobertura NÃO é nota. Linha tracejada = média geral deste recorte ({pct(resumo.get('percentualPontuacao'))}), calculada com os resultados confirmados aqui analisados. Não é média do ENEM, do Brasil, do Acre ou do município.",
        'info',
    ))
    story.append(Spacer(1, 2*mm))
    ...
    if turmas:
        data=[['Turma','Estudantes','Desempenho','Cobertura','Faixa da turma']]
        for x in turmas:
    ...
    story += section('4. Participação por dia', 'O total previsto é o universo de estudantes do simulado naquele recorte. Presentes variam por dia; ausência confirmada não é erro nem baixa cobertura artificial.')
    part=visual.get('participacaoPorDia') or dashboard.get('participacaoPorDia') or []
    if part:
        resumo_dias = '; '.join(f"{inteiro(x.get('dia'))}º dia: {inteiro(x.get('presentes'))} de {inteiro(x.get('previstos'))} presentes" for x in part)
        story.append(callout(
            'Como ler esta tabela',
            f"{inteiro(resumo.get('participantes'))} é o universo de estudantes do recorte, não o número presente em cada dia. {resumo_dias}. Quem faltou continua pertencendo ao simulado, mas as questões do dia ausente não são tratadas como erro nem entram artificialmente na cobertura.",
            'info',
        ))
        story.append(Spacer(1, 2*mm))
        data=[['Dia','Previstos no dia','Presentes','Ausentes confirmados','Participação']]
        for x in part:
    ...
    story += section('5. Questões por faixa de acerto', 'A distribuição ajuda a perceber se a dificuldade está concentrada em alguns itens ou espalhada pela prova. Aqui, "Evidência insuficiente" refere-se a QUESTÕES com base abaixo do mínimo; não a estudantes.')
    ...
    if fq:
        ...
        story.append(Spacer(1, 2*mm))
        story.append(callout('O que significam as faixas das questões', 'Estas faixas classificam o desempenho de cada QUESTÃO, e não o estudante. Elas servem para localizar itens com maior ou menor percentual de acerto e orientar revisão pedagógica.', 'info'))
        story.append(Spacer(1, 2*mm))
        story.append(tabela_faixas_questoes(cfg))
    ...
    story.append(callout(
        'Como ler as classificações de habilidade',
        f"Sustentada = pelo menos {inteiro(cfg.get('minimoQuestoesIndicador', 2))} questão(ões) no indicador e base mínima configurada de respondentes. Prioritária = desempenho abaixo de {pct(cfg.get('percentualAtencao', 50))} com evidência suficiente. Em desenvolvimento = de {pct(cfg.get('percentualAtencao', 50))} a menos de {pct(cfg.get('percentualConsolidado', 70))}. Consolidada = a partir de {pct(cfg.get('percentualConsolidado', 70))}, sempre restrita a este simulado. Um único item gera leitura indicativa, não conclusão de domínio ou dificuldade.",
        'info',
    ))
    story.append(Spacer(1, 2*mm))
    ...
    story.append(callout('O que significa variação em p.p.', 'Pontos percentuais medem a diferença direta entre dois percentuais. Exemplo: passar de 36% para 41% corresponde a +5 pontos percentuais. A comparação não representa ganho de pontos TRI e só é válida para os estudantes e alvos em comum.', 'info'))
    story.append(Spacer(1, 2*mm))
    ...
    story += [PageBreak()]
    story += section('9. Como o Axoriin interpreta os resultados', 'Os parâmetros abaixo são critérios internos de gestão pedagógica. Eles tornam a leitura consistente e evitam conclusões frágeis, mas não são cortes oficiais do ENEM nem parâmetros da TRI.')
    story.append(callout(
        'Princípio metodológico',
        'O Axoriin separa três coisas: desempenho (quanto foi acertado), cobertura (quanto dos dados aplicáveis está confirmado) e participação (em quais dias o estudante esteve presente). Só depois verifica se a evidência é suficiente para classificar aluno, questão ou habilidade. Isso evita transformar ausência ou dado faltante em baixo desempenho.',
        'info',
    ))
    story.append(Spacer(1, 3*mm))
    story.append(tabela_parametros_diagnostico(cfg))
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph('Significado das faixas de desempenho individual', H2))
    story.append(tabela_faixas_estudantes(cfg))
    story.append(Spacer(1, 3*mm))
    story.append(callout('Uso responsável', 'As faixas organizam prioridades de intervenção e acompanhamento. Elas não devem ser usadas isoladamente para rotular estudantes, definir proficiência oficial ou tomar decisões de alto impacto sem outras evidências escolares.', 'warn'))
    return story
...
def build_story_habilidades_enem(payload: dict[str, Any]) -> list:
    ...
    cards = [
        ...
        ('Diagnóstico sustentado', str(len(sustentadas)), 'habilidade medida por múltiplos itens + base mínima'),
        ('Habilidades prioritárias', str(len(prioritarias)), 'retomada curricular com evidência suficiente'),
        ('Cobertura das respostas', pct(resumo.get('coberturaPercentual')), 'confirmadas entre as respostas aplicáveis'),
    ]
    story += [metric_cards(cards), Spacer(1, 4 * mm)]
    story.append(callout(
        'Guia rápido de termos',
        f"Habilidade trabalhada = apareceu em ao menos uma questão do simulado. Leitura indicativa = evidência ainda insuficiente para conclusão sustentada, especialmente quando há um único item. Diagnóstico sustentado = atende ao mínimo de {inteiro(cfg.get('minimoQuestoesIndicador', 2))} questão(ões) por indicador e à base mínima de respondentes. Prioritária = abaixo de {pct(cfg.get('percentualAtencao', 50))}; consolidada = a partir de {pct(cfg.get('percentualConsolidado', 70))}, sempre como leitura interna deste simulado.",
        'info',
    ))
    story.append(Spacer(1, 2*mm))
    ...
    story += section('6. Critérios de leitura e por que eles existem', 'O relatório específico mantém as mesmas regras metodológicas do diagnóstico geral, mas restringe a apresentação à Matriz ENEM.')
    criterio = (
        ...
        "Isso reduz o risco de declarar domínio ou dificuldade curricular a partir de um único item. Habilidade medida por um item permanece indicativa. Mapeamentos aproximados são identificados explicitamente."
    )
...
def build_story(payload: dict[str, Any]) -> list:
    ...
    cards = [
        ('Estudantes no recorte', str(inteiro(resumo.get('participantes'))), f"{inteiro(resumo.get('turmas'))} turma(s)"),
        ('Desempenho confirmado', pct(resumo.get('percentualPontuacao')), 'percentual de acertos; brancos entram como zero'),
        ('Acerto nas respostas marcadas', pct(resumo.get('percentualAcerto')), 'acertos somente entre alternativas A-E marcadas'),
        ('Cobertura das respostas', pct(resumo.get('coberturaPercentual')), f"{inteiro(resumo.get('observadas'))} de {inteiro(resumo.get('aplicaveis'))} respostas aplicáveis"),
        ('Classificação individual', str(inteiro(resumo.get('alunosBaseAdequada'))), f"participação completa + cobertura mínima de {pct(cfg.get('minimoCoberturaIndividual', 80))}"),
        ('Participação parcial', str(inteiro(resumo.get('alunosParticipacaoParcial'))), 'ausência confirmada; dias/áreas realizados continuam válidos'),
        ('Base incompleta', str(inteiro(resumo.get('alunosBaseIncompleta'))), 'participou, mas faltam dados para conclusão global'),
    ]
    story += [metric_cards(cards, columns=4), Spacer(1, 2 * mm)]
    story.append(callout(
        'Diferença entre presença, cobertura e classificação',
        'Estudantes no recorte é o universo total. Participação por dia mostra quem esteve presente. Cobertura mede respostas confirmadas entre as aplicáveis. A classificação individual só é atribuída quando há participação completa e dados suficientes.',
        'info',
    ))
    story.append(Spacer(1, 2 * mm))
    story.append(callout(
        'Guia rápido para apresentação',
        f"Desempenho confirmado = percentual de acertos no recorte, com brancos valendo zero. Cobertura = percentual dos dados aplicáveis que está confirmado; não é nota. Classificação individual = somente estudantes com participação completa e ao menos {pct(cfg.get('minimoCoberturaIndividual', 80))} de cobertura. A média geral do recorte é uma referência interna dos estudantes analisados, não média oficial do ENEM.",
        'info',
    ))
    story.append(Spacer(1, 2 * mm))
    ...
    quality_data = [
        ...
        ['Estudantes com base completa', inteiro(resumo.get('alunosBaseCompleta')), 'Participação completa, cobertura de 100% e sem língua pendente.'],
        ['Classificação individual disponível', inteiro(resumo.get('alunosBaseAdequada')), f"Participação completa, cobertura de pelo menos {pct(cfg.get('minimoCoberturaIndividual', 80))} e sem língua pendente."],
        ['Participação parcial', inteiro(resumo.get('alunosParticipacaoParcial')), 'Ausência confirmada em ao menos um dia; não entra na classificação global, mas os dias/áreas realizados permanecem válidos.'],
        ['Base incompleta', inteiro(resumo.get('alunosBaseIncompleta')), 'Participou, mas a cobertura/evidência ainda não sustenta conclusão global individual.'],
        ['Estudantes sem opção de língua', inteiro(resumo.get('alunosSemOpcaoIdioma')), 'O zero afeta o resultado, mas não vira prioridade curricular.'],
    ]
    story.append(table(quality_data, [56 * mm, 24 * mm, 100 * mm]))

    dist = dashboard.get('distribuicaoAlunos') or []
    if dist:
        story.append(Paragraph('Situação dos estudantes no diagnóstico', H2))
        story.append(p('Intervenção imediata, Prioridade alta, Em atenção, Em desenvolvimento e Consolidado são faixas de DESEMPENHO. Participação parcial e Base incompleta são situações de DADOS e não devem ser lidas como baixo desempenho.', SMALL))
        story.append(Spacer(1, 1.5*mm))
        story.append(tabela_faixas_estudantes(cfg))
        story.append(Spacer(1, 2*mm))
        total = max(1, sum(inteiro(x.get('quantidade')) for x in dist))
        data = [['Situação', 'Estudantes', '% do recorte']]
        backgrounds = {}
        for idx, x in enumerate(dist, 1):
            q = inteiro(x.get('quantidade'))
            data.append([nivel_aluno_label(x.get('nivel')), q, pct(100 * q / total)])
    ...
    if turmas:
        ...
        data = [['Turma', 'Estudantes', 'Desempenho', 'Acerto marcado', 'Cobertura', 'Dif. geral', 'Leitura']]
        backgrounds = {}
    ...
    story.append(p('Esta seção documenta não apenas os valores usados, mas a finalidade de cada trava metodológica. Os limiares são critérios internos de organização pedagógica e não cortes oficiais do ENEM.', SMALL))
    story.append(Spacer(1, 2*mm))
    story.append(tabela_parametros_diagnostico(cfg))
    story.append(Spacer(1, 3*mm))
    ...
    if faixas:
        ...
        data = [['Faixa', 'Regra']]
        for x in faixas:
            data.append([nivel_aluno_label(x.get('nivel')), txt(x.get('regra'))])
