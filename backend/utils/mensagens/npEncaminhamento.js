...
function montarEmailNP({ aluno, turma, notaAtual, linkAgendamento, contatoEscola }) {
  const nome = String(aluno || '').trim();
  const turmaTxt = String(turma || '').trim();
  const nota = Number(notaAtual || 0);
  const notaFmt = isFinite(nota) ? nota.toFixed(2) : String(notaAtual || '');

  const assunto = `[CMDPII/CZS] Encaminhamento ao Núcleo Psicossocial – ${nome} (${turmaTxt})`;
...
Atenciosamente,
Coordenação de Ensino – CMDPII/CZS`;
  ...
  <table role="presentation" width="620" cellpadding="0" cellspacing="0" style="width:620px;max-width:100%;background:#ffffff;border-radius:14px;box-shadow:0 10px 30px rgba(0,0,0,.06);overflow:hidden;border:1px solid #eceff3;">
    <tr>
      <td style="background:#8B0000;padding:16px 22px;color:#fff;">
        <div style="font-weight:700;font-size:18px;letter-spacing:.2px;">CMDPII/CZS – Coordenação de Ensino</div>
        <div style="opacity:.9;font-size:13px;">Notificação de Encaminhamento ao Núcleo Psicossocial</div>
      </td>
    </tr>
    <tr>
      <td style="padding:22px;">
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="margin:14px 0 16px 0;">
          <tr>
            <td style="background:#fff7f7;border:1px solid #f3d1d1;border-radius:10px;padding:14px;">
              <div style="font-size:14px;color:#5a0c0d;font-weight:700;margin-bottom:6px;">Base normativa</div>
              <div style="font-size:14px;color:#5a0c0d;line-height:1.5;">
                <em>Art. 51 — “Encaminhar ao Núcleo Psicossocial (NP) e registrar no histórico. O NP deve informar os responsáveis.”</em>
              </div>
            </td>
          </tr>
        </table>
        ...
        <ul style="margin:0 0 16px 18px;padding:0;font-size:14.5px;line-height:1.55;">
          <li>Encaminhamento ao <b>NP</b> para escuta e orientação;</li>
          <li><b>Registro</b> no histórico escolar;</li>
          <li>Contato do <b>NP</b> com os responsáveis;</li>
          <li>Acompanhamento conjunto com a <b>monitoria</b>.</li>
        </ul>
        ...
        <p style="margin:0;font-size:13px;color:#6b6f76;">Mensagem automática do Sistema Escolar – CMDPII/CZS.</p>
      </td>
    </tr>
    <tr>
      <td style="background:#fafbfc;border-top:1px solid #eceff3;padding:14px 22px;color:#6b6f76;font-size:12.5px;">
        Caso já tenha ocorrido o atendimento no NP, desconsidere esta mensagem.
      </td>
    </tr>
  </table>
